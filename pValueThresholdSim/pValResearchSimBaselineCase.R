...
#
myFileName         <- "pH0_0.8VL2.Rdata"       # file for saving output
...
#
#   set effect size of new labs 
#
		deltaD                 <- rgamma(L,1,rate = -log(1-pH0True)/deltMin)
		myDelta[indxDeath,]    <- matrix(rep(deltaD,times=maxHypotheses),ncol=maxHypotheses)	    
	  }
#
#  Record averages across labs along with confidence intervals for positives and negatives
#	  
	  truePosList[i,]  <- truePosList[i,] + c(sum(labs$truePos)/nLabs-1/sqrt(nLabs),sum(labs$truePos)/nLabs,sum(labs$truePos)/nLabs+1/sqrt(nLabs)) 	  
...
#
# record medians and quartiles across labs for effort, value, and number of hypotheses, averaging over simulation replicate
#	  
...
#
# record median "true effect size" deltas for each lab at the final time
#
	for (iii in 1:nLabs){
		 medDelt[iii]<- median(myDelta[iii,(1:labs$nHypotheses[iii])])
	}
#
# record final-time quantities for each lab and each simulation replicate
#	
